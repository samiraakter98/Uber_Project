...
--to check set_payment procedure				
BEGIN
	SET_PAYMENT('6','2');
END;
/
...
CREATE OR REPLACE FUNCTION CHECK_PROMO(U_ID IN VARCHAR2)
RETURN NUMBER IS
		COUNT_NUM NUMBER;
		BOOL NUMBER;
		STATUS VARCHAR2(20);
...
BEGIN
		...
		IF COUNT_NUM > 0 THEN 
				FOR P IN (SELECT PROMO_CODE_NO_ID FROM GETS WHERE USER_ID LIKE U_ID)
				LOOP
					 SELECT PROMO_STATUS INTO STATUS FROM PROMO WHERE PROMO_CODE_NO LIKE P.PROMO_CODE_NO_ID;
					 IF STATUS LIKE 'ACTIVE' THEN
							BOOL:= 1;
							EXIT;
					 END IF;
...
--procedure to update promo status
CREATE OR REPLACE PROCEDURE SET_PROMO_STATUS(U_ID IN VARCHAR2) IS
		EXPIRED_DATE DATE;
		CURR_DATE DATE;
		STATUS VARCHAR2(20);
BEGIN
		...
		LOOP
	      SELECT PROMO_STATUS INTO STATUS FROM PROMO WHERE PROMO_CODE_NO LIKE R.PROMO_CODE_NO_ID;
				SELECT EXPIRED_AT INTO EXPIRED_DATE FROM PROMO WHERE PROMO_CODE_NO LIKE R.PROMO_CODE_NO_ID;
				IF CURR_DATE > EXPIRED_DATE THEN
						UPDATE PROMO SET PROMO_STATUS = 'INACTIVE' WHERE PROMO_CODE_NO LIKE R.PROMO_CODE_NO_ID;
				END IF; 
		END LOOP;
END;
/

--to check set_promo_status procedure				
BEGIN
	SET_PROMO_STATUS('1');
END;
/
...
--END;
--/
--UPDATE PROMO SET PROMO_STATUS = 'ACTIVE' WHERE PROMO_CODE_NO = '11';
--SELECT SYSDATE + 7
--FROM DUAL;
